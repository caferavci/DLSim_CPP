...
public:
    CAgent_Simu() : agent_vector_seq_no{ -1 }, path_toll{ 0 }, departure_time_in_min{ 0 }, m_bGenereated{ false }, m_bCompleteTrip{ false },
        path_travel_time_in_min{ 0 }, path_distance{ 0 }, diversion_flag{ 0 }
    {
    }
                ...
                if (format_type.find("column") != string::npos)  // or muliti-column
                {
                    ...
                    if (st)
                    {
                        ...
                        while (true)
                        {
                            ...
                            if (demand_value >= 1)
                            {
                                critical_OD_volume += demand_value * loading_scale_factor;
                                critical_OD_count += 1;
                    ...
                    if (parser.OpenCSVFile(file_name, false))
                    {
                        ...
                        while (parser.ReadRecord())
                        {
                            ...
                            if (format_type.compare("path") == 0)
                            {
                                ...
                                path_counts++;
                                sum_of_path_volume += agent_path_element.volume * loading_scale_factor;
    ...
    if (parser_link.OpenCSVFile("link.csv", true))
    {
        while (parser_link.ReadRecord())  // if this line contains [] mark, then we will also read field headers.
        {
            ...
            for (int tau = 0; tau < assignment.g_number_of_demand_periods; ++tau)
            {
                ...
                parser_link.GetValueByFieldName("cycle_time", link.VDF_period[tau].cycle_length,false,false);
...
void g_output_assignment_result(Assignment& assignment)
{
    ...
    else
    {
            ...
            fprintf(g_pFileLinkMOE, "link_id,from_node_id,to_node_id,time_period,volume,travel_time,speed,speed_ratio,VOC,queue,density,geometry,");
            ...
            for (int i = 0; i < g_link_vector.size(); ++i)
            {
                ...
                for (int tau = 0; tau < assignment.g_number_of_demand_periods; ++tau)
                {
                    ...
                    fprintf(g_pFileLinkMOE, "%s,%d,%d,%s,%.3f,%.3f,%.3f,%.3f,%.3f,0,0,\"%s\",",
                        g_link_vector[i].link_id.c_str(),
                        g_node_vector[g_link_vector[i].from_node_seq_no].node_id,
                        g_node_vector[g_link_vector[i].to_node_seq_no].node_id,
    ...
    else if(assignment.assignment_mode >= 1)  //UE mode, or ODME, DTA
    {
       ...
       fprintf(g_pFilePathMOE, "agent_id,o_zone_id,d_zone_id,path_id,information_type,agent_type,demand_period,demand_requests,volume,subarea_flag,sensor_flag,toll,travel_time,distance,path_code,node_sequence,link_sequence,time_sequence,link_travel_time_sequence,geometry\n");
                        ...
                        if (p_column_pool->od_volume > 0 || assignment.zone_seq_no_2_info_mapping.find(orig) != assignment.zone_seq_no_2_info_mapping.end())
                        {
                            ...
                            for (it = it_begin;it != it_end; ++it)
                            {
                                ...
                                if(assignment.assignment_mode >= 1 )
                                {
                                    ...
                                    fprintf(g_pFilePathMOE, "%d,%d,%d,%d,%d,%s,%s,%.2f,%d,%d,%d,%.1f,%.4f,%.4f,%s,",
                                                         count,
            ...
            for (int t = 1; t < assignment.g_number_of_intervals_in_min; ++t)
            {
                ...
                if (t % (sampling_time_interval ) == 0)
                {
                    ...
                    fprintf(g_pFileLinkMOE, "%s,%s,%s,%d,%d,%s,%d,%.3f,%.3f,%.3f,%s_%s,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,\"%s\",",
                        ...
                        g_time_coding(assignment.g_LoadingStartTimeInMin + time_in_min - sampling_time_interval/60.0).c_str(),
...
void g_output_simulation_agents(Assignment& assignment)
{
    ...
    else if (assignment.assignment_mode >= 1)  //UE mode, or ODME, DTA
    {
        ...
        fprintf(g_pFileAgent, "agent_id,o_zone_id,d_zone_id,path_id,#_of_links,diversion_flag,agent_type,demand_period,volume,toll,travel_time,distance,speed,departure_time_in_min,departure_time_slot_no,\n");
                            ...
                            for (it = it_begin; it != it_end; ++it)
                            {
                                ...
                                // assignment_mode = 1, path flow mode
                                {
                                    ...
                                    for (int vi = 0; vi < it->second.agent_simu_id_vector.size(); ++vi)
                                    {
                                        ...
                                        time_stamp = assignment.g_DemandPeriodVector[tau].starting_time_slot_no * MIN_PER_TIMESLOT + pAgentSimu->departure_time_in_min;
                                        ...
                                        fprintf(g_pFileAgent, "%d,%d,%d,%d,%d,%d,%s,%s,1,%.1f,%.4f,%.4f,%.4f,%.4f,%d",
                                            pAgentSimu->agent_id,
                                            g_zone_vector[orig].zone_id,
                                            g_zone_vector[dest].zone_id,
                                            it->second.path_seq_no,
                                            ...
                                            pAgentSimu->path_distance, speed, departure_time_in_min, departure_time_in_slot_no);
...
void g_output_simulation_result(Assignment& assignment) 
{
    ...
    else if (assignment.assignment_mode >= 1)  //UE mode, or ODME, DTA
    {
        ...
        fprintf(g_pFilePathMOE, "agent_id,o_zone_id,d_zone_id,path_id,diversion_flag,agent_type,demand_period,volume,toll,travel_time,distance,node_sequence,link_sequence,time_sequence,waiting_time_in_simu_interval,geometry\n");
                                ...
                                // assignment_mode = 1, path flow mode
                                {
                                    ...
                                    for (int vi = 0; vi < it->second.agent_simu_id_vector.size(); ++vi)
                                    {
                                       ...
                                       //     continue;
                                        ...
                                        float vehicle_travel_time = path_time_vector[pAgentSimu->path_link_seq_no_vector.size() - virtual_link_delta] - path_time_vector[virtual_link_delta];
                                        
                                        // some bugs for output link performances before
                                        fprintf(g_pFilePathMOE, "%d,%d,%d,%d,%d,%s,%s,1,%.1f,%.4f,%.4f,",
                                            pAgentSimu->agent_id,
                                            g_zone_vector[orig].zone_id,
                                            g_zone_vector[dest].zone_id,
                                            it->second.path_seq_no,
